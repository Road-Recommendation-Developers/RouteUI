...
public class FreeActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    public List<Map<String, Object>> getData(List<Course> Freetable,String  week){
        //尝试批量处理
        int day,time=0;
        String taskname = null,location;
        //构建35个map,从而进行编辑
        for(int i=0;i<35;i++){
            Map<String, Object> map = new HashMap<String, Object>();//偏移问题day+time*7
            for(Course course:Freetable)
            {
                if(course.getWeeks().equals(week)){
                    ...
                    {
                        taskname="Free";
                        location='@'+"to be choose from free classroom";
